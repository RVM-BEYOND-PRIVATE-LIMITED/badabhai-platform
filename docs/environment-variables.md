...
| `REFERRAL_ATTRIBUTE_MAX_PER_IP_PER_HOUR`   | Per-IP cap / rolling UTC hour on the worker-authed `POST /referrals/attribute` referral-attribution hook (ADR-0022 Amendment 1). Fail-closed via `IpRateLimit` (Redis outage → 429).                                                      | `20`         | config     |
| `AGENCY_PAYOUTS_ENABLED`                   | Launch gate for the AGENCY supply-money surface (KYC + earnings + payout, ADR-0022 Amendment 2). **Default OFF ⇒ every `/payer/agency/kyc\|earnings\|payouts` route is a neutral 404 and NO financial PII is collected.** `booleanFromString` (falsey stays OFF). Flipping ON presupposes the legal/DPDP sign-off on live KYC; money stays MOCK regardless (`PAYMENTS_ENABLE_REAL` is the separate §7 gate). | `false` (off) | config     |
| `AGENCY_PAYOUT_UNLOCK_BASIS_INR`           | The ₹ unlock-revenue anchor a commission accrues on (owner-ratified, ADR-0022 Amdt 2). Stamped per accrual row.                                                                                                                          | `40`         | config     |
| `AGENCY_PAYOUT_RATE_BPS`                   | Commission rate in basis points (2500 = 25%, owner-ratified). Stamped per accrual; `accrual = floor(basis × rate/10000)` = ₹10 by default.                                                                                              | `2500`       | config     |
| `AGENCY_PAYOUT_WINDOW_DAYS`                | First-touch attribution window: a granted unlock accrues only within this many days of the invite's `attributed_at`.                                                                                                                     | `90`         | config     |
| `AGENCY_PAYOUT_MIN_THRESHOLD_INR`          | Minimum requestable payout — a request below this ₹ total is blocked (`agency_payout.blocked{below_threshold}`).                                                                                                                          | `500`        | config     |
...
| `AI_SPEND_REDIS_URL`             | Shared store for the TD27 spend ledger. **Unset ⇒ in-process backend** (caps enforce PER PROCESS — the deliberate dev/test/single-process default, _not_ a failure); set ⇒ Redis backend (caps enforce GLOBALLY across Uvicorn workers). **Validated at startup**: the value must begin with `redis://`, `rediss://` or `unix://` — a malformed URL (e.g. a missing scheme) fails `Settings()` at boot with an error naming this variable and omitting its value, rather than aborting boot from inside the redis library. Shape only, **no** connectivity check. Fails **closed**: a well-formed but unreachable URL boots normally, then blocks real calls per call (`spend_store_unavailable` ⇒ mock), bounded by a 2s connect/socket timeout. The selected backend is logged **once at startup** (a FastAPI lifespan hook constructs the ledger at boot — the line names this variable, never its value); it is **also** reported by `/health` as `spend_store`, **but only when the service is unlocked** — once `AI_INTERNAL_TOKEN` is set, `/health` trims to `{status, service, service_auth_enabled}` and the boot log is the only signal. **Renamed from `REDIS_URL` (AI-ENV-1, hard cut — no alias):** it collided with the Node API's `REDIS_URL`, which is mandatory infrastructure with an incompatible meaning. | _(optional)_ | **SECRET** (may carry credentials) |
