...
| `redis`                     | **Yes**                                                                                                          | `PING` over the existing BullMQ ioredis client — **connectivity ONLY**. It says nothing about persistence, volume identity, or whether the keyspace still has anything in it: a brand-new EMPTY redis PONGs, so a wipe of the worker session store reads perfectly healthy here. Survival is checked at deploy time instead (CD-7 in `.github/workflows/ci.yml`); see [staging session durability](./ops/staging-session-durability.md) |
