...
| `AGENCY_PAYOUTS_ENABLED` | 🌐 | false | OK (off — **launch gate**, ADR-0022 Amdt 2/PR #508: while OFF every `/payer/agency/{kyc,earnings,payouts}` + `/ops/agency-kyc/*` route is a neutral 404, no financial PII collected. Flip presupposes legal/DPDP sign-off on live KYC collection + the 2 fix-before-flip items in TD39; money stays mock regardless of this flag — `PAYMENTS_ENABLE_REAL` is the separate real-money gate) |
| `AGENCY_PAYOUT_UNLOCK_BASIS_INR` / `AGENCY_PAYOUT_RATE_BPS` / `AGENCY_PAYOUT_WINDOW_DAYS` / `AGENCY_PAYOUT_MIN_THRESHOLD_INR` | 🌐 | 40 / 2500 / 90 / 500 | OK (owner-ratified 2026-07-23 economics — 25%×₹40/unlock, 90d window, ₹500 minimum; stamped per accrual row so a later edit never rewrites history) |
