...
|---|---|---|---|---|---|---|---|
| **BP-1** | **Additive admin-guarded read endpoints** (workers, companies, agencies, jobs, applications, credits, transactions) | **P0** | **DONE** 2026-08-04 (PR #574) | AP-1 pass | Foundation proven | 8 `@UseGuards(AdminAuthGuard, AdminRolesGuard)` GET routes in `admin-entities.*`, all `read_entities` (ADR-0025 §3.1 row 2 — signed with the matrix, never coded until now). `InternalServiceGuard` NOT widened; ops console untouched. Migration 0064 adds the 4 keyset indexes. | 13/13 mutations caught; live-verified on a real DB incl. the PII scan over every response body |
| **BP-3** | **Admin directory + capability matrix reads** | **P0** | **DONE** 2026-08-04 (PR #578) | BP-1 | Milestone 5 needed them | `GET /admin/admins` (**FACELESS** — owner ruling, no name/email; `manage_admins`, super_admin only) + `GET /admin/capabilities` (`read_entities`, matrix DERIVED via `can()` so the portal carries no second copy of the authorization table). No migration. | 9/9 mutations; live-verified incl. the no-identity sweep |
| **BP-2b** | **Admin finance reads** (credit position, platform-wide ledger, payment orders) | **P0** | **DONE** 2026-08-04 (PR #576) | BP-1 | Milestone 4 needed them | 3 `read_entities` GET routes at `/admin/finance/*`; every money-bearing response carries the payments posture so mock ₹ can never read as revenue. Migration 0065 adds 2 keyset indexes. | 10 caught + 1 equivalent mutant; live-verified incl. the mock/real labelling |
