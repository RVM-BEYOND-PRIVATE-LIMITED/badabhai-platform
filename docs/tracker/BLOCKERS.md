...
| ~~**Migrations `0062` + `0063` unapplied**~~ ✅ **RESOLVED 2026-08-03 — applied successfully by Prakash.** `0062` (job-posting `suspended` state + `previous_status`) and `0063` (admin `name_enc` + `mfa_secret_enc`). The ADR-0037 suspension writes and the ADR-0038 admin login now have their columns. | DB/Infra | ~~P0~~ **CLEARED** | — | — | — | Prakash | No |
| **Admin Portal foundation gate not yet run on a clean environment** — the 13-step [runbook](../admin-foundation-verification-runbook.md) is written and its two blocking defects are fixed, but it has **not been executed** end-to-end from an empty DB. Under CLAUDE.md invariant #10 the ADR-0038 foundation is therefore *not* proven. | QA/Backend | **P1** | Start of ADMIN-4..8 UI work | — | — | Admin Portal thread | No — run it first; any failure is a backend defect handed back |
