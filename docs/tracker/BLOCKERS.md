...
| ~~**Admin Portal foundation gate not run**~~ ✅ **CLEARED 2026-08-03 — executed, 13/13 PASS** on a database created by `docker compose down -v`. It did NOT pass first time: a **P0** blocker was found at step 2 (26 migrations REVOKE on Supabase-only roles → the chain died on `0004` with zero tables created) and fixed. Evidence: [QA_EVIDENCE](QA_EVIDENCE.md). ADMIN-4 is unblocked. | QA/Backend | ~~P1~~ **CLEARED** | — | — | — | Backend thread | No |
