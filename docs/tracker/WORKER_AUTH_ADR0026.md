...
## Divyanshu's 6 tasks (Phase 2 + 3) — ALL LANDED (#167/#168; snapshot below kept for the spec trail)

Each rolled into Phase 2/3 above. The per-task **Status** lines below were the pre-build
verification snapshot and are superseded: **T1–T6 are DONE/MERGED** (T1 tables+migrations
0029/0030 via #167/#168 · T2 scrypt PinHasher #168 · T3 lockout ladder #168, hardened by
#175 · T4 pin/device routes #167/#168 · T5 reset + device binding #167/#168 · T6 suites
green + bb-security-review PASS per phase). Acceptance criteria remain the reference.
...
- **Status:** ✅ DONE — merged #167/#168 (migrations 0029/0030; RLS FORCE+REVOKE carried).
...
- **Status:** ✅ DONE — merged #168 (`pin-hasher.service.ts`, scrypt + env pepper; D7 resolved).
...
- **Status:** ✅ DONE — merged #168; hardened by #175 (cycle-0 flush + /pin/reset per-IP cap).
...
- **Status:** ✅ DONE — merged #167/#168 (`pin.controller.ts`, `devices.controller.ts`).
...
- **Status:** ✅ DONE — merged #167/#168 (SIM-swap gate = trusted-device requirement on /auth/pin/verify).
...
- **Status:** ✅ DONE — suites green per phase; bb-security-review PASS (Phase 3: 2 reviews + re-check).
...
---
_Update the phase/task % only with a [QA_EVIDENCE](QA_EVIDENCE.md) row._
