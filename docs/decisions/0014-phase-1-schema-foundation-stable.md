...
1. **The foundation is real and working.** The Phase-1 profiling spine (workers → consent →
   ...
   locked (TD20).
2. **The schema is still growing — on purpose.** It is now **21 tables** (migrations through
   `0015`), with the last several added this week under Phase-2 ADRs (0009 swipe, 0010
   contact-unlock, 0012 job-postings). A hard "no more changes" freeze contradicts the live
   roadmap. So the right commitment is **additive-only + versioned**, not "frozen."
...
- **The contract downstream teams (API, AI, mobile) can rely on:** the 21 tables below and
  their shipped event payloads will not change shape under them without an ADR.

## The 21 tables this stabilizes
...
**Phase-2 additive, already landed (7):** `job_postings` · `jobs` · `applications` ·
...
| 2 | All migrations `0000`–`0015` generated with rollback notes | ✅ | `packages/db/migrations/` |
| 3 | PII isolated to `workers`, encrypted at rest | ✅ | ADR-0004 |
| 4 | Spine-wide RLS + REVOKE lock on all 21 tables, no-drift test | ✅ | TD20, `rls-spine.e2e.test.ts` |
| 5 | Events/ai_jobs/audit_logs are PII-free (ids/hashes only) | ✅ | invariant 2 |
| 6 | Event payloads versioned, never mutated | ✅ | invariant 8 |
| 7 | Schema doc reconciled to reality (21 tables) | ✅ | `docs/schema/README.md` (this PR) |
...
> versioned + ADR change policy above (21 tables, items 1–9 closed)?**
>
...
- **Q: Are there open risks I'm accepting?**
  ...
  today), and embeddings first-use (Q8). None changes the 21-table contract.
