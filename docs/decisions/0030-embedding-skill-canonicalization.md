...
- **Status:** **Proposed** (awaits owner ratification — the human gate). Architecture gate only — this ADR draws the seam and the phased rollout; it produces **no code, schema, migration, extension, import, or embedding call**. Implementation is handed to the engineer agents per phase (TAX-1…TAX-9), each its own PR behind its own enter-gate.
- **Date:** 2026-07-14
...
- **Builds on / reconciles (verified against the repo, 2026-07-14):**
  ...
  - **`packages/db/src/schema.ts`** — **36 tables today; no `VECTOR` column exists and `pgvector` is not enabled.** There is a frozen, unused `embeddings` table (ADR-0001 #4) for a *future* matching use; this ADR does **not** use it (that is a ranking artifact — see §Invariant-#4 boundary). TAX-1 adds pgvector + 3 new tables → 39.
...
- The frozen `embeddings` table (ADR-0001 #4) — reserved for a *future matching* use — is **not** used by this ADR. TAX-1's `skill_alias.embedding` is a **canonicalization** artifact, deliberately kept distinct.
...
|---|---|---|---|
| **(a)** | **pgvector = new datastore capability (§3 stack).** | **devops** confirms the Supabase project tier has `pgvector` available; enable via `CREATE EXTENSION IF NOT EXISTS vector` in the migration; **migration-reviewer** pass. If unavailable → escalate, do **not** proceed. | **TAX-1** |
| **(b)** | **Embedding provider = real calls + spend (invariant #5, SG-4).** | Embedding generation is a **real provider call**. It stays behind `AI_ENABLE_REAL_CALLS=false` by default, requires a key, and runs **staging-first**. Record the embedding model + dimension (768 working assumption — confirm in TAX-3) and the per-1k-token cost in the cost tracker. | **TAX-3, TAX-4** |
| **(c)** | **ESCO / O\*NET licensing (redistribution terms).** | Confirm and **record in this ADR / the register**: ESCO's exact licence (CC-BY-family — attribution + share terms) and O\*NET's public-domain-US status (+ attribution request). Do **not** import a source until its redistribution terms are confirmed. | **TAX-2** (import) |
...
|---|---|---|
| **0 — this ADR** | sources + seam + invariant-#4 boundary + §7 gates + phased design. No code. | — (you are here; Status: **Proposed** → owner ratifies) |
| **1 — DB: pgvector + `skill` / `skill_alias` / `unresolved_phrase`** | migration 0036 on a head of 0035: `CREATE EXTENSION vector`; immutable `skill_id`; `skill_alias.embedding vector(768)` + **HNSW (cosine)** index + a btree on `domain_id`/`skill_id`; `unresolved_phrase` (pseudonymized text, counts, status). Additive; no shipped column touched. | ADR **Accepted** **AND** §7(a) — devops confirms pgvector + migration-reviewer |
...
- **SG-1 / §2 #2 (no raw PII).** Skill phrases carry no identity PII (they describe capability, not the worker). `unresolved_phrase` stores **pseudonymized** text only; no phone/name/employer ever lands in `skill`/`skill_alias`/`unresolved_phrase`, embeddings, events, `ai_jobs`, `audit_logs`, or logs.
- **SG-2 / §2 #3 (pseudonymize fail-closed).** Every phrase passes through the pseudonymizer **before** it is embedded (a real provider call) — same fail-closed gate as every other LLM path, even though skill phrases are expected PII-free. If pseudonymization blocks, no embedding call is made.
- **SG-3 / §2 #4 (LLMs assist, never decide).** The LLM emits phrases; the **vector layer** assigns the `skill_id`; the model **never invents a `skill_id`**. Canonicalization is not ranking; no LLM or embedding enters the RANK path (see §Invariant-#4 boundary).
...
- **SG-5 / §2 #8 (additive, versioned, expand→migrate→contract).** All 3 tables + the extension are **additive**; no shipped column/event is mutated. `skill_id` is immutable/never-reused; the vocabulary is versioned; every phase carries a rollback note.
...
4. **Reuse the frozen `embeddings` table / build skills straight into ranking.** Rejected: that conflates canonicalization with ranking and would introduce an embedding signal into the live path — a separate decision requiring its own ADR (see §Invariant-#4 boundary). This ADR deliberately keeps skills out of RANK.
...
- ADR-0001 (`0001-mvp-infra-decision.md`) — the frozen `embeddings` table (a future *ranking* artifact) this ADR deliberately does **not** use.
...
- `packages/db/src/schema.ts` — 36 tables today (no `VECTOR`, no pgvector); TAX-1 adds the extension + 3 tables → 39.
