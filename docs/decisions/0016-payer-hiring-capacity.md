...
- **Status:** **Accepted — PHASE-0 decisions human-signed 2026-06-17 (G0 + D1).** This ADR records
  the contract; the **build is authorized as additive Phase-2 monetization work** under the
  [ADR-0014](0014-phase-1-schema-foundation-stable.md) additive-only policy. **No code is written by
  this ADR.** Real recurring billing, a production payer surface, and a `job_postings`→reach bridge
  remain out / escalations (see §Escalations).
- **Date:** 2026-06-17
...
> state lives on `posting_plans` — the subject is the plan. To confirm at build with the
> system-architect.
...
- **Additive / backward-compatible** — one new nullable-friendly enum value on `posting_plans.status`,
  one new table (`payer_capacity`), one new catalog product kind; **no shipped column/event/payload
  mutated** (ADR-0014 / invariant 8).
- **Atomic, no cap-bypass** — the cap check + plan write is one advisory-locked transaction (ADR-0010
  F-2), so N concurrent activations for one payer can never exceed the cap.
- **Mock payments only** — `PAYMENTS_ENABLE_REAL=false`; `real_call` stamped honest.

## Security caveat (must be documented at build, not assumed enforced)
...
surface ships on the shared secret.** (Cross-ref TD33.)
...
## Consequences / follow-ups (to log as tech-debt at build)
...
## Build plan (additive; authorized, routed to agents at build time)
1. **database-architect** — `posting_plans.status += 'paused'` (additive CHECK update); new
   ...
   migration + rollback; RLS backlog.
2. **backend-engineer** — extend `@badabhai/pricing` with the `capacity` product kind (ops-editable,
   fail-closed); the atomic per-payer chokepoint in `buyPlan` (advisory lock, pause-at-cap); the
   capacity purchase/upgrade flow with auto-resume; new v1 events; reuse `payment.*` (mock).
3. **qa-engineer** — N-concurrent-activation never exceeds the cap (atomic); pause-at-limit;
   auto-resume on purchase/upgrade; faceless/no-PII; events emitted; deterministic resume order.
4. **security-engineer** — faceless aggregation; document the PayerAuthGuard advisory caveat; mock only.
5. **code-reviewer + technical-writer** — additive-only; update registers; this ADR's status → built.
