...
| 13 | ~~Five~~ ~~Six~~ live migrations unrecorded in the journal | ~~owner — production write~~ | 🟢 **CLOSED 2026-08-20 — journal is 83/83 and `db:migrate` will attempt no DDL.** All six (`0076`–`0080`, `0082`) adopted after full-depth verification. `0082` needed new machinery: a **third** out-of-band apply landed a hand-pinned `created_at=1787230000000`, moving the watermark PAST `0082` and flipping it from *replayed* to *silently skipped forever* — while adoption still refused its `DO $$` block. Resolved by an **effect verifier** (42 catalog assertions), which is stronger evidence than the text parse, not weaker. Idempotency proven live: the re-run selected 0. **One residue:** the orphan row itself — a migration on production from a checkout that never reached `main`, carrying a table this repo does not model. Locked, so no exposure; its author owes the reconciliation |
