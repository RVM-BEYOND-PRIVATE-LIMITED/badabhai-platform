...
| 2026-06-15 | **Contact Unlock + Reveal — routed-disclosure monetization spine** (Phase-0 design, **Proposed — awaiting sign-off**). Fail-closed disclosure ordering (consent→caps→payment→grant→routed-reveal); raw phone read only at the final routed-reveal, never in events/`ai_jobs`/`audit_logs`/logs. Mock credits (real gateway behind `PAYMENTS_ENABLE_REAL=false`), in-app relay (no telephony provider) in alpha, separate `employer_sharing` DPDP consent, per-worker caps at a single chokepoint. PII-disclosure threat model written; bb-security-review = CONCERNS with F-1/F-2 build-blockers folded in. 6 decisions are RVM's to sign off; real keys/raw reveal escalate | ADR | [0010-contact-unlock-and-reveal.md](../decisions/0010-contact-unlock-and-reveal.md) |
...
- [0010 — Contact Unlock + Reveal (routed-disclosure monetization spine)](../decisions/0010-contact-unlock-and-reveal.md) — *Proposed (2026-06-15; Phase-0 design + threat model, awaiting human/RVM sign-off)*
