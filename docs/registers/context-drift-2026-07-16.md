...
| **D-2** ✅ **RESOLVED** | ~~**Voice notes 30–120s cannot transcribe.** Platform accepts **120s** (`MAX_VOICE_NOTE_SECONDS`); `stt.py:50` `SARVAM_SYNC_MAX_SECONDS = 30.0` and `:190-194` raises *"batch STT not implemented"*. **The 2-minute product promise and the STT transport disagree by 4×.** Fails closed, silently unusable.~~ **RESOLVED 2026-07-17** per the owner ruling ([team-decisions.md](./team-decisions.md) 2026-07-17 item 8: *"BUILD IT PROPERLY = ASYNC STT"*) — built as **chunked sync**, NOT the provider's batch API (Sarvam's batch contract is not derivable from this repo; only the sync endpoint + the 30s limit are known, so the proven pattern was taken over a guessed API shape). New [`app/audio_chunk.py`](../../apps/ai-service/app/audio_chunk.py) splits the stored `.m4a` (AAC→ADTS, frame-exact, no re-encode — **pure python, because the ai-service has no container image to add ffmpeg to**) / `.wav` into **<29.5s** segments; segments ride the SAME sync endpoint at concurrency **2**; transcripts concatenate **in segment order**. A 120s note = **5 chunks ≈ ₹1.25**, under the ₹6/user/day cap (~4 full notes/user/day), reserved→reconciled **per chunk** on the TD27 ledger via a new optional `worker_ref` (Zod↔Pydantic lockstep, additive). `AiService.transcribe` fetch budget 8s→**270s** (that ONE call; all others keep 8s). The `"batch STT not implemented"` raise is **GONE**; **>120s** (the platform cap) is now the only duration refusal. **The mock is duration-agnostic** — the 30s guard had been applied to a path that never calls a provider (the absurdity this row named). Gate ORDER verified: chunks never escape the adapter, so pseudonymize still gates the **FULL** transcript, never a chunk. Real Sarvam creds stay **§7**. See the **[ADR-0029 D-2 addendum](../decisions/0029-voice-audio-at-rest-and-upload-seam.md#addendum-2026-07-17--d-2-30120s-notes-transcribe-via-chunkedasync-stt)**. ⚠️ **GATES ON THE REAL FLIP:** (1) ~~PR #392 must merge first~~ **#392 MERGED 2026-07-17** — chunking introduces 4 seams per 120s note, and a phone split at a seam used to **leak to LLM input** un-blocked (`_RESIDUAL_DIGITS_RE = \d{7,}` sees two 5-digit halves); #392's digit-count normalization closes the bare-space/period/comma/ellipsis class (an earlier claim here that boundary-split numbers "are still masked" was **FALSE — withdrawn**; it held only for a bare join-space). **TWO seam leaks remain, both `xfail(strict=True)`, neither patched from the D-2 PR:** (a) the **Devanagari danda `।` U+0964** — a **gap in #392's separator class** (S-4 folded in dashes/zero-width/middot but no Devanagari; the "13/13 shapes" matrix had no danda case), which matters because a Hindi STT seam is exactly where a danda lands — **one-character fix, escalated to the pseudonymize owner**; (b) **[R30](./risks-register.md)'s word-split** (`98765 haan 43210`), **open by design** — a proximity net cannot distinguish it from `salary 15000 se 18000` and would destroy real salary data. (2) **[TD59](./tech-debt-register.md)** — ~14s client poll vs a ~4-min real 120s note. (3) the **₹0.25/chunk rate is an estimate** pending the first invoice. **Sarvam must not be armed while R30 is open.** Spend abuse closed in review: the reservation (from client-declared duration) was **decoupled** from the call count (from the file's own tables) — a 200KB crafted `.m4a` declaring 200,000s issued **6,780 calls (₹1,695)** against a ₹0.50 reservation; now bounded by reserve-the-structural-bound + a file-duration check at the source + `MAX_CHUNKS_PER_NOTE=5`, and `_MAX_AAC_FRAMES` cut 20M→10k (508B→320MB amplification → 4.7:1). | ~~P1~~ **DONE** |
