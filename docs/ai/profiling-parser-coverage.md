# Deterministic profiling parser — coverage measurement (POST parser widening)

> **This revision is NOT measurement-only.** Earlier revisions changed zero
> runtime files; the revision that produced these numbers WIDENS
> `app/profiling/signals.py` — role cues, state/region answers to the PREFERRED
> location question, and the Devanagari/misspelling 'anywhere' family. Overall
> acceptance moves for the first time; see "What the parser widening changed"
> for the per-topic before/after, the negatives that make it safe, and the four
> classes it deliberately did NOT close.
>
> **`CNC`, bare `operator` and `cnc oprator` STILL resolve nothing — by ruling,
> not omission.** Every operator role in the closed set names a machine family,
> so resolving them would have to invent one; minting a generic id instead is a
> measured RANKING regression (reach `scoreRole`: 0.0 'different trade' vs 0.4
> 'trade not stated'). Finding 1 carries the argument and a test locks it.
>
> Prior baselines, each measured at the time: `6d23419` (the `pre-#426` column),
> then **#426** (`fea207d`, P1 profiling correctness), **#412** (`41d0cb7`,
> TAX-WELD-1) and **#429** (`64d4001`, issue #424 — salary/availability promoted
> to MUST_ASK). None of them is re-derived here; they survive only as labelled
> historical columns, never as current behaviour.
>
> **Read finding 7 first** — an ESSENTIAL topic can be marked answered without
> ever being asked, and the completeness signal cannot see it. Unchanged.
>
...
- **auto-closed topics** — `skills` is marked answered by 16 of the
  49 role/machine answers without ever being asked, so in practice
...
| `controllers` | 13/22 (59%) | 13/22 (59%) | +0 |
...
| --- | --- | --- | --- | --- | --- |
| `role` | `V M C operator` | `_topic not marked_` | `VMC Operator` | `VMC Operator` | **CLOSED** |
| `role` | `seter ka kaam` | `_topic not marked_` | `CNC Setter-Operator` | `CNC Setter-Operator` | **CLOSED** |
| `role` | `मैं वीएमसी ऑपरेटर हूँ` | `_topic not marked_` | `VMC Operator` | `VMC Operator` | **CLOSED** |
| `role` | `प्रोग्रामर` | `_topic not marked_` | `CNC Programmer` | `CNC Programmer` | **CLOSED** |
| `preferred_locations` | `Gujarat mein` | `_topic not marked_` | `['Gujarat']` | `['Gujarat']` | **CLOSED** |
| `preferred_locations` | `South India` | `_topic not marked_` | `['South India']` | `['South India']` | **CLOSED** |
| `preferred_locations` | `NCR` | `_topic not marked_` | `['NCR']` | `['NCR']` | **CLOSED** |
| `preferred_locations` | `जहाँ भी काम मिले` | `_topic not marked_` | `flexible` | `flexible` | **CLOSED** |
| `preferred_locations` | `Gujarat ya Maharashtra dono chalega` | `_topic not marked_` | `['Gujarat', 'Maharashtra']` | `['Gujarat', 'Maharashtra']` | **CLOSED** |

- `V M C operator` — spacing variant; `_ROLES` is substring-matched and cannot see it
- `seter ka kaam` — `setter` with one `t`
- `मैं वीएमसी ऑपरेटर हूँ` — Devanagari form of a gazetteer entry that already exists in Latin
- `Gujarat mein` — a STATE is a real answer to 'kahan kaam kar sakte hain?' — unlike for CURRENT location, where the engine should go on to ask for the city
- `जहाँ भी काम मिले` — Devanagari 'anywhere'; `\b` does not work after a matra, so it needs its own boundary form
- `Gujarat ya Maharashtra dono chalega` — ALL areas, not the first: the PR #488 review measured first-match-wins dropping the second choice while closing the topic against re-asking

### The negatives that make it safe

A widening is only as good as what it still REFUSES. The `recorded` column is
live, so a future over-match shows up here as a value: **62 of 62** rows hold.

**45 of these are marked ADVERSARIAL**: they were MEASURED as
...
| --- | --- | --- | --- | --- |
| `role` | `CNC` | — | — | a family-of-families (VMC/HMC/lathe/grinder are all CNC) |
| `role` | `operator` | — | — | the function without the machine family |
| `role` | `ऑपरेटर` | — | — | same, in the other script |
| `role` | `lathe operator` | — | — | the inference itself is GONE — an honest gap again, worth -1 acceptance |
| `role` | `lathe chalata hu` | — | — | same |
| `role` | `grinder operator` | — | — | same |
| `role` | `angle grinder chalata hu` | — | — | ADVERSARIAL: an ANGLE grinder is a handheld tool, not a CNC grinding machine |
| `role` | `TIG aur MIG welding karta hu, grinder bhi chalata hu` | — | — | ADVERSARIAL: a welder with a grinder is not a CNC grinding operator |
| `role` | `welder hu, lathe chalata hu kabhi kabhi` | — | — | ADVERSARIAL: the cue used to preempt `_assign_welding_role`; with no cue at all the welding gate is reached exactly as on main |
| `role` | `lathe operator ka helper hu` | — | — | ADVERSARIAL: the HELPER, not the operator |
| `role` | `mere bhai lathe operator hai` | — | — | ADVERSARIAL: a third party's role |
| `role` | `lathe operator mere saath kaam karta tha` | — | — | ADVERSARIAL round 3: the matched pair of a row the round-2 blocklist DID block — `ke saath` was listed, `mere saath` was not |
| `role` | `pitaji lathe chalate hai` | — | — | ADVERSARIAL round 3: `papa` was listed, `pitaji` and `chacha` were not |
| `role` | `chacha lathe chalate hai` | — | — | ADVERSARIAL round 3 |
| `role` | `lathe operator ki zarurat hai` | — | — | ADVERSARIAL round 3: `requirement` was listed, `zarurat` was not |
| `role` | `lathe operator ki salary kitni hoti hai` | — | — | ADVERSARIAL round 3: an INTERROGATIVE — nothing blocked questions at all |
| `role` | `lathe operator ka kaam kaisa hota hai` | — | — | ADVERSARIAL round 3 |
| `role` | `ek lathe operator ko jaanta hu` | — | — | ADVERSARIAL round 3 |
| `role` | `lathe operator ke under kaam kiya` | — | — | ADVERSARIAL round 3 |
| `role` | `pehle jahan tha wahan lathe operator tha` | — | — | ADVERSARIAL round 3 |
| `role` | `hamari company me lathe hai, operator ki jagah khali hai` | — | — | ADVERSARIAL: a VACANCY being described |
| `role` | `lathe operator banna chahta hu` | — | — | ADVERSARIAL: an aspiration |
| `role` | `lathe chalane ki training li hai` | — | — | ADVERSARIAL: an EDUCATION answer, not a role claim |
| `role` | `lathe operator ka kaam mujhe nahi aata` | — | — | ADVERSARIAL: a denial |
| `role` | `seater cover lagata hu` | — | — | near-miss on the `set[ae]r` variant |
| `role` | `v mc` | — | — | not a spelled-out acronym |
| `role` | `वीएमसी नहीं चलाता` | — | — | denial, Devanagari — the same masked text `_ROLES` reads |
| `preferred_locations` | `Bihar nahi jaunga` | — | — | a REFUSED state is not a preference |
| `preferred_locations` | `Gujarat mein nahi jaunga` | — | — | same, one filler word |
| `preferred_locations` | `Kerala mein bilkul bhi nahi jaunga` | — | — | ADVERSARIAL: two filler words walked through the 3-word backward mask |
| `preferred_locations` | `West Bengal me kaam karne ki koi ichha nahi hai` | — | — | ADVERSARIAL: 'no wish to work there' recorded as a preference |
| `preferred_locations` | `Bihar me kaam karne ka mann nahi hai` | — | — | ADVERSARIAL |
| `preferred_locations` | `Odisha ki taraf jaana mujhe pasand nahi` | — | — | ADVERSARIAL |
| `preferred_locations` | `Punjab wale bulate hai par mai nahi jaunga` | — | — | ADVERSARIAL |
| `preferred_locations` | `Kerala bahut door hai, nahi ja sakta` | — | — | ADVERSARIAL: the negator is in a DIFFERENT clause |
| `preferred_locations` | `Bihar ke alawa kahin bhi` | `flexible` | `flexible` | ADVERSARIAL REGRESSION: 'anywhere EXCEPT Bihar' was `flexible` on main and an earlier cut turned it into `['Bihar']` — the one state ruled out, topic closed |
| `preferred_locations` | `Bihar ke alaawa kahin bhi kaam kar sakta hu` | `flexible` | `flexible` | ADVERSARIAL round 3: one extra `a` defeated the round-2 exclusion list |
| `preferred_locations` | `Bihar ke alaava kahin bhi kaam kar sakta hu` | `flexible` | `flexible` | ADVERSARIAL round 3 |
| `preferred_locations` | `Bihar ke alawaa kahin bhi` | `flexible` | `flexible` | ADVERSARIAL round 3 |
| `preferred_locations` | `Bihar chhodke kahin bhi kaam kar sakta hu` | `flexible` | `flexible` | ADVERSARIAL round 3: `chhod ke` was listed, the unspaced `chhodke` was not |
| `preferred_locations` | `Bihar chodke kahin bhi kaam kar sakta hu` | `flexible` | `flexible` | ADVERSARIAL round 3 |
| `preferred_locations` | `Bihar hatake kahin bhi kaam kar sakta hu` | `flexible` | `flexible` | ADVERSARIAL round 3 |
| `preferred_locations` | `Bihar ke sivay kahin bhi kaam kar sakta hu` | `flexible` | `flexible` | ADVERSARIAL round 3: the listed `ke siva` could not reach `sivay` |
| `preferred_locations` | `Bihar ke siwaay kahin bhi kaam kar sakta hu` | `flexible` | `flexible` | ADVERSARIAL round 3 |
| `preferred_locations` | `Bihar ke atirikt kahin bhi` | `flexible` | `flexible` | NEVER enumerated anywhere — passes because the guard is a positive requirement, which is the whole point of the rewrite |
| `preferred_locations` | `Bihar se hu` | — | — | where the worker is FROM |
| `preferred_locations` | `main Bihar ka hu` | — | — | ADVERSARIAL round 3: the round-2 origin list had `se hu`, not `ka hu` |
| `preferred_locations` | `ghar Bihar me hai` | — | — | ADVERSARIAL round 3 |
| `preferred_locations` | `gaon Bihar me hai` | — | — | ADVERSARIAL round 3 |
| `preferred_locations` | `Bihar mera home town hai` | — | — | ADVERSARIAL round 3 |
| `preferred_locations` | `Bihar me paida hua` | — | — | ADVERSARIAL round 3 |
| `preferred_locations` | `mere papa Kerala me rehte hain` | — | — | ADVERSARIAL round 3: the third-party guard was applied to the ROLE path only |
| `preferred_locations` | `Bihar me tha, ab Gujarat me kaam chahiye` | — | — | ADVERSARIAL: first-match-wins recorded the state the migrant LEFT. Abandoned whole, so the real 'Gujarat' answer is lost too — a GAP, and the correct trade |
| `preferred_locations` | `kahin bhi ja sakta hu, abhi Gujarat me kaam kar raha hu` | `flexible` | `flexible` | ADVERSARIAL round 3: a REGRESSION vs main — the state they are IN replaced the anywhere they offered |
| `preferred_locations` | `company Gujarat me hai, main kahin bhi jaa sakta hu` | `flexible` | `flexible` | ADVERSARIAL round 3 |
| `preferred_locations` | `Maharashtra me salary kam hai, kahin bhi bhej do` | `flexible` | `flexible` | ADVERSARIAL round 3 |
| `preferred_locations` | `Punjab me kaam milega to theek, warna kahin bhi` | `flexible` | `flexible` | ADVERSARIAL round 3 |
| `preferred_locations` | `mera bhai Kerala me hai, main kahin bhi ja sakta hu` | `flexible` | `flexible` | ADVERSARIAL round 3 |
| `preferred_locations` | `Maharashtra mein kahin bhi` | `flexible` | `flexible` | REVERTED to main's value: an earlier cut recorded ['Maharashtra'], ranking the state above the anywhere. Precedence now goes to `flexible` |
| `preferred_locations` | `set UP karta hu` | — | — | 'UP' in caps inside 'set UP' — why the 2-letter abbreviations are not read here |
| `preferred_locations` | `south side me rehta hu` | — | — | 'south' alone is not a region |
| `preferred_locations` | `Gujarat me kaam?` | — | — | a QUESTION is not an answer — found by re-probing the allow-list, since both `me` and `kaam` are filler and it otherwise passed |
...
| `machines` | `वीएमसी` | `role`=VMC Operator | the widening's deliberate ASYMMETRY: the Devanagari cue is a ROLE cue only, so `machines` (ESSENTIAL) stays open and gets asked — Latin 'VMC' closes both |
...
**Scope of this number.** It counts only CORPUS fixtures a human labelled
`reject`, and the corpus is capped at 25 per topic (`role` and
`preferred_locations` are both at the cap). The 20+ fabrications the PR #488
review measured therefore could NOT be added here — they live in "The negatives
that make it safe" above, which pins each one to an EXACT expected value rather
than merely to not-accepted. Read the two together; neither alone is the whole
fabrication picture.
...
Item 2 was the parser widening and is now **DONE in part** — see "What the
parser widening changed". Everything else below is still the SHAPE of a fix,
in the order the data ranks it, and none of it is implemented.
...
5. **`kabhi` matching `abhi`** (finding 6.2) — DONE by the #424 follow-up; kept
   in the list because the report's probe table still measures it every run.
5b. **Devanagari beyond `role` / `preferred_locations`** — the widening covered
   only the two topics it was scoped to. `experience` (`5 साल`), `skills`,
   `education` (`आईटीआई किया है`), `controllers` (`फैनुक`) and `salary_current`
   (`पंद्रह हज़ार`) all still return nothing in Devanagari. Note `\b` does NOT
   work after a Devanagari matra — use `signals._dev`, and probe it, or the
   pattern is silently dead.
...
| `role` | `प्रोग्रामर` | yes | `role` | accept |
...
| `role` | `lathe nahi chalaya, helper hu` | NO | `skills` | reject |
...
| `machines` | `वीएमसी` | NO | `role` | accept |
...
| `preferred_locations` | `Gujarat mein` | yes | `preferred_locations` | accept |
| `preferred_locations` | `South India` | yes | `preferred_locations` | accept |
| `preferred_locations` | `NCR` | yes | `preferred_locations` | accept |
...
| `preferred_locations` | `जहाँ भी काम मिले` | yes | `preferred_locations` | accept |
...
| `preferred_locations` | `Bihar nahi jaunga` | NO | — | reject |
