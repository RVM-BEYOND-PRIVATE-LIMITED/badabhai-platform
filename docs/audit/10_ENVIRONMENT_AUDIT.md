...
**Drift since this audit ran (added 2026-08-19, #997)**: the **154** above was exact when measured and is left as measured — the §5 template-coverage figure derived from it ("123 of 154") is not reproducible from the same grep today, so re-pointing one number without the other would only make the pair look maintained. `serverEnvSchema` now declares **159** fields: `OTP_MAX_SENDS_PER_IP_PER_HOUR`, `CHAT_ABANDON_AFTER_SECONDS` and `CHAT_ABANDON_SWEEP_INTERVAL_HOURS` landed without a pass here, and #997 adds `WORKER_FEEDBACK_PER_MINUTE`/`WORKER_FEEDBACK_PER_HOUR` (§2). Neither compose file forwards any of the five — as with every other rate cap in this schema, they run on their `.default(...)` in staging and production.
