...
| `worker_feedback` | In-app feedback the worker typed themselves (#997) — free text, optional category, `x-app-build` stamp. **Holds worker PII by expectation rather than by accident** — see §1 | Y (0079) | Y — `POST /workers/me/feedback` writes, `GET /admin/feedback` reads |
