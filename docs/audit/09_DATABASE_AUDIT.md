...
**A fourth column on the same table, and it is NOT in the class above — `worker_feedback.screen_context` (`0081`).** Client-supplied, and NOT stored: the request edge (`resolveScreenTemplate`) matches the untrusted value against `WORKER_APP_SCREEN_TEMPLATES` — the worker app's own 28 route constants, re-derived from `apps/worker-app/lib/router.dart` — and binds THE TABLE'S CONSTANT or `NULL`. No byte a caller chose can reach the column, which is why, unlike `message`, it *is* allowed onto the events spine (`feedback.submitted.screen_context`) and into the API log line. The spine re-asserts membership itself (`z.enum`), so a second emitter added later without the resolver fails closed there.

⚠ THIS REPLACED A DENYLIST, and the history is the reason the guarantee is now stated as absolute. The first normalizer substituted id SHAPES anchored to whole segments and let `/jobs/id-<uuid>/apply` and `/w/9876543210-ravi` through verbatim; the second matched runs anywhere and still let `/u/dGVzdEBleGFtcGxlLmNvbQ` (base64url of an email address) and `/x/AKIAIOSFODNN7EXAMPLE` through, because no structural rule separates an opaque token from a route word. Membership of a closed set is a different KIND of control, not a longer list. The residual is now MAINTENANCE, not leakage: a screen the app adds and the table does not know reports as `NULL` forever — caught in CI by `screen-template-table.contract.test.ts`, which reads the app's router, and at runtime by a value-free WARN on the submit path. No shipped client sends the field yet, so every row is currently `NULL`.
...
| `worker_feedback` | In-app feedback the worker typed themselves (#997) — free text, optional category, `x-app-build` stamp, and (`0081`) a `screen_context` drawn from the app's own closed set of screen names. **Holds worker PII by expectation rather than by accident** — see §1 | Y (0080) | Y — `POST /workers/me/feedback` writes, `GET /admin/feedback` reads |
...
**Production-guarded reference/catalog seeds (dry-run-by-default; idempotent; PII-free):** `seed-jobs.ts`, `seed-questionnaire.ts`, `seed-skills.ts`, `seed-job-domains.ts`, `seed-question-packs.ts`, `seed-match-vocabulary.ts` (the last three via the `match-v1-cli` harness), `normalize-job-domain-aliases.ts`, `embed-skill-aliases.ts`, `embed-job-domain-aliases.ts`. **Updated 2026-08-20:** `seed-skills.ts`, `embed-skill-aliases.ts`, `promote-skills.ts` and the whole `match-v1-cli` family now gate on `opsGuard`, which classifies the CONNECTION STRING and requires `--i-am-authorised-to-write-to-production` plus `OPS_ALLOW_PRODUCTION=<script>` before a production write; `MATCH_V1_PROD_CONFIRM` is retired. The remainder still key on `NODE_ENV` alone and are enumerated, with reasons and a severity order, in `packages/db/src/ops-guard-adoption.test.ts`.
