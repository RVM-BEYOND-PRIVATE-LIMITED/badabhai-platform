...
|---|---|---|---|
| `ci.yml` | `push`/`pull_request` → `main` | **Yes — `ci-required` is the sole required check on `main`** | Lint/typecheck/test/build, ai-service pytest/ruff, E2E onboarding, Flutter gates, SAST, dependency audit, Docker build+push, Lightsail deploy, worker + payer APK build+release |
