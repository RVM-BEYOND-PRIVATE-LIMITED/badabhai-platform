...
| Abusive-language ENFORCEMENT *(OPEN — the only open call in this document)* | Detection is not in question; what to *do* on a hit is. Three candidates: flag-only · flag + strip from payer-facing surfaces · block the turn. **Owner's recommendation on #747: flag + strip from payer surfaces, never block a valid turn.** Divyanshu builds leg (b) only after this is ruled; leg (a) is not blocked by it. |
