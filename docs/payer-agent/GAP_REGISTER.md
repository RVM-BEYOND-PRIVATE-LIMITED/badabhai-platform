...
| **GAP-XC-06** | ci/infra | Deploy | **`apps/payer-web` is built by no workflow and deployed by none**; `staging-cd.yml:172` skips the Next apps | a deployment path | **P1** | — | Decide hosting (Vercel / container / Lightsail), add the pipeline, set `PAYER_API_URL` + CORS origin | Smoke test against the deployed origin |
