...
/**
 * MONEY-ROUTE body PARITY (FE-3) — the payer-authed buy bodies mirror the backend
...
describe("money-route bodies — mirror the backend PayerBuy{Plan,Boost}Schema (no payer_id, id on PATH)", () => {
  it("every allowed money-body key is exactly { tier, coupon } — never payer_id/amount/id", () => {
    for (const body of [
      { tier: "standard" },
      { tier: "pro", coupon: "PROMO" },
      { tier: "all_candidates" },
      { tier: "all_candidates", coupon: "SAVE10" },
    ]) {
