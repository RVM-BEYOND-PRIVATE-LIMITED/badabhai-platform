...
void main() {
  ...
  testWidgets(
    'the Alerts tab shows NO employer/company name, ₹/pay, or phone text',
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(find.text('Aapki application aage pahunch gayi.'), findsOneWidget);

      // Right icon…
      expect(find.byIcon(Icons.send_rounded), findsOneWidget);

      // …and the green tone: v3 `successGreen` on `successBg` (§1.1), read from
      // the token layer rather than a hex, so a token move cannot pass silently.
      final Icon icon = tester.widget<Icon>(find.byIcon(Icons.send_rounded));
      expect(icon.color, OnboardingColors.successGreen);
  ...
  testWidgets('pulling down the alert list refetches (pull-to-refresh)', (
    ...
    verify(
      () => repo.list(),
    ).called(1); // exactly one MORE list() from the pull
  });
