...
///
/// #1200 — the screen is READ-ONLY: the in-app purchase surface was removed
/// (store IAP policy / mobile-payments rule), so there is no `buyPack` path to
/// test here. Credit packs are bought on the payer WEB portal.
...
void main() {
  ...
  test('a failed balance read leaves the balance unknown, never a fake 0',
    ...
    // The reads now fire concurrently (one round trip, not three), so the
