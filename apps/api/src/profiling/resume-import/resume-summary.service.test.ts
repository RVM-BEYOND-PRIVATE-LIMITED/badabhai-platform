...
describe("ResumeSummaryService — summarize (best-effort, never shown in chat)", () => {
  ...
  it("sends the storage KEY with the 14 enabled kinds, never the document", async () => {
    const { svc, ai } = setup();
    ...
    // 14 enabled forms, not the 21 declared kinds.
    expect(sent.role_kinds).toHaveLength(14);
