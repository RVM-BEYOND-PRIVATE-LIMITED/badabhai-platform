...
describe("JobsService.getWorkerVisibleJob — the ADR-0024 SHOW projection", () => {
  it("returns EVERY contract field, mapped snake_case, values intact", async () => {
    ...
    expect(out).toEqual({
      ...
      pay_min: 18000,
      pay_max: 25000,
      // #1648 — the poster's own net-vs-gross claim, passed through verbatim.
      pay_type: "in_hand",
  ...
  it("passes nulls through HONESTLY on every nullable field (absent data, never fabricated)", async () => {
    ...
    expect(out).toEqual({
      ...
      pay_min: null,
      pay_max: null,
      // #1648 — NULL is "the poster did not state it", and the card then shows the band
      // with no pay-type pill. It is never defaulted to `gross` to fill the hole.
      pay_type: null,
