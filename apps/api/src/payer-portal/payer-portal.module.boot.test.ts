 ...
 *
 * NOTE (R16 / LC-1): the payer-self DISCLOSURE surface (unlock/reveal/credits) is NOT in
 * this module — it is the canonical `/unlocks` controller retrofitted in-place onto
 * PayerAuthGuard in UnlocksModule. This module holds only the auth + reach controllers.
 */
...
describe("PayerPortalModule wiring (cross-module DI regression guard)", () => {
  it("imports PayersModule + ReachModule (the reused foundations)", () => {
  ...
  it("declares the auth + reach payer controllers", () => {
  ...
  it("provides the auth service, OTP store, and all three login channels", () => {
  ...
  it("the reach controller is class-guarded by PayerAuthGuard; auth is PUBLIC", () => {
    // PayerReachController: every route requires a payer session.
