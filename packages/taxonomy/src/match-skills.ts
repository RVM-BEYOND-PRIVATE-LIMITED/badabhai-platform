...
export const ATTRIBUTE_TO_MATCH_SKILLS: Record<string, readonly MatchSkillId[]> = {
  ...
  // =========================================================================
  // PROMOTABLE GROWTH BATCH — Q1 dispositions, OWNER-RATIFIED 2026-08-26
  // =========================================================================
  // The 96 skills of the phase-9d promotion scope. Before Q1 none of them had a key here,
  // and the exhaustiveness test could not see that: its universe was SKILL_CORPUS, and none
  // of these is a corpus skill. They could have promoted to `active` and reached nothing at
  // match time with no test failing. See `promotable-skills.ts`.
  //
  // FIVE are MATCHED. The rest are INTENTIONALLY_UNMATCHED, and the empty arrays are the
  // point exactly as they are above: an explicit "stays an attribute", not an oversight.
  //
  // The triage measured what an automated pass would have produced: for 61 of 96 the nearest
  // already-triaged neighbour is mapped, so similarity would have proposed 61 mappings. Its
  // most confident ones were wrong — ducting_installation -> plumber @ 0.827 (a duct carries
  // air), visual_defect_identification -> quality_inspector @ 0.803 (every operator does it).
  // The owner ratified 5. Rationale per skill: `q1-disposition-triage.ts` in @badabhai/db.
  //
  // 62 of the 96 sit in trade families the vocabulary does not represent at all — electrical,
  // warehouse, masonry, HVAC, automotive service, battery, assembly, sheet metal. They are
  // unmatched because THERE IS NOTHING TO MATCH THEM TO, not because they were judged
  // attributes. Creating `mskill_*` for those trades is a separate owner decision.
  ...
  skill_sanitary_fixture_installation: ["mskill_plumber"],
  skill_leak_repair_in_water_lines: ["mskill_plumber"],
  skill_solvent_cement_jointing: [],
  skill_drain_cleaning_and_unclogging: ["mskill_plumber"],
  skill_pipe_bending: [],
  ...
  skill_hardness_testing: ["mskill_quality_inspector"],
  skill_non_destructive_testing_of_castings: ["mskill_quality_inspector"],
