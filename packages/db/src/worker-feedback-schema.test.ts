/**
 * Worker app feedback (migration 0079) — schema contract tests.
 *
...
const MIGRATION = readFileSync(join(__dirname, "../migrations/0080_worker_feedback.sql"), "utf8");
...
describe("0079 — the drizzle model and the SQL describe the same table", () => {
  it("models exactly the six columns the migration creates", () => {
    ...
    for (const name of columns.keys()) {
      expect(MIGRATION, `${name} must appear in the CREATE TABLE`).toContain(`"${name}"`);
    }
  });
   ...
   */
  it("keeps category and app_build nullable, message and worker_id not", () => {
  ...
  it("declares the three CHECKs and the three indexes in both the model and the SQL", () => {
    for (const name of [
...
describe("0079 — the bounds are pinned to @badabhai/types, and this is the only thing pinning them", () => {
  /**
...
describe("0079 — the privacy posture, which is hand-appended and therefore fragile", () => {
  /**
...
describe("0079 — additive and reversible", () => {
  /**
