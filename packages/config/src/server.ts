...
export const serverEnvSchema = z.object({
  ...
  //     ON PURPOSE — see the long note in `assertAuthConfig` for the history and the accepted
  //     consequence. In one line: with this on, ANY authenticated worker can irreversibly delete
  //     their own account with no OTP and no grace, and the shipped gate is which APK carries the
  //     button — which is a client-side gate, not an authorization boundary.
   ...
   *
   * ENTIRELY OPTIONAL, AND EMPTY IS THE SHIPPED POSTURE (owner decision, 2026-08-27). Empty
   * means EVERY authenticated worker may use the seam, in every environment including production;
   * non-empty restricts it to those ids. Nothing anywhere requires this to be set — the boot
   * refusal that briefly did was removed in the same decision, and `assertAuthConfig` carries the
   * history. Set it only if you want the narrowing; the product does not depend on it.
   *
  ...
  // PER-PHONE SEND CAP, ONE ROLLING UTC HOUR.
  //
  // ⚠ THIS KNOB IS NOT WORKER-ONLY, AND THAT IS WHY IT STAYS AT 5. `admin-otp.service.ts:201`
  // and `payer-otp.service.ts:212` read it for their own per-account hourly caps — two
  // authenticated staff/customer surfaces with a different threat model from an unauthenticated
  // worker's phone. It is now the ADMIN + PAYER number and nothing else: the worker path moved to
  // `WORKER_OTP_MAX_SENDS_PER_HOUR` below (#1421), which is what the withdrawn 2026-09-04 owner
  // ruling actually needed. Raising THIS one still loosens both portals, so it stays at 5.
  //
  // ITS DEFAULT IS DELIBERATELY UNCHANGED. A deployment that sets `OTP_MAX_SENDS_PER_HOUR`
  // today keeps the exact meaning it had before #1421 on the two surfaces that still read it —
  // the split is additive, never a re-pointing of an existing name (§10).
  OTP_MAX_SENDS_PER_HOUR: z.coerce.number().int().positive().default(5),
  // PER-PHONE SEND CAP FOR THE WORKER PATH ONLY, ONE ROLLING UTC HOUR (#1421).
  //
  // WHY IT IS A SECOND KNOB RATHER THAN A BIGGER SHARED ONE. `OTP_MAX_SENDS_PER_HOUR` above is
  // read by three services, so there was no way to give a worker more room inside an hour
  // without also loosening the admin portal and the payer portal. That sharing is what withdrew
  // the 2026-09-04 owner ruling of 10/hour; this knob is that ruling, landed on the one surface
  // it was meant for. `otp.service.ts` reads this and nothing else does.
  //
  // WHAT 10 BUYS, and why 5 was the wrong number for a worker. At the shipped 30s resend
  // cooldown, five sends is under three minutes of a failed onboarding — and PIN reset draws on
  // the SAME per-phone budget (`pin.service.ts` sends through `AuthService.requestOtp`, and the
  // counters are keyed on the phone hash with NO purpose segment). So a worker who fails to sign
  // in and then reaches for forgot-PIN spends one budget for both, and runs out at the worst
  // possible moment. The DAILY cap was raised to 30 for this reason; it helps across a day and
  // does nothing inside the hour a worker is actually stuck in.
  //
  // IT MUST STAY STRICTLY BELOW OTP_MAX_SENDS_PER_DAY, for the same reason the shared knob must,
  // and `config.test.ts` pins that too. Both counters INCR on every accepted send, so an hourly
  // limit at or above the daily one can never refuse a request the daily one would not also
  // refuse in the same UTC day — dead configuration that still reads like a live control.
  WORKER_OTP_MAX_SENDS_PER_HOUR: z.coerce.number().int().positive().default(10),
  ...
  // (5 is "how many texts may one number receive an hour", sized against paid Fast2SMS
  // spend), while this one is a per-NETWORK abuse backstop ("how many humans may sign in
  // from one egress IP"). They are different questions and only ever coincided at 5 by
  ...
  //
  // THIS WAS THE HALF OF THAT RULING THAT COULD SHIP AT THE TIME. The same ruling proposed
  // raising the hourly cap 5 -> 10 and that half was withdrawn, because OTP_MAX_SENDS_PER_HOUR is
  // shared with the admin and payer OTP services and could not be moved for workers alone. #1421
  // landed it properly: the worker hourly budget is now WORKER_OTP_MAX_SENDS_PER_HOUR (10).
  //
  // 30 > BOTH HOURLY CAPS BY CONSTRUCTION, not coincidence — WORKER_OTP_MAX_SENDS_PER_HOUR (10)
  // for the worker path this knob actually governs, and OTP_MAX_SENDS_PER_HOUR (5) for the two
  // portals. See the note on those knobs for why an equal pair silently kills the hourly cap.
  // `config.test.ts` pins both inequalities.
...
export function assertAuthConfig(
  ...
  // ── THE IMMEDIATE HARD-DELETE SEAM: #1187 -> #1264 -> HERE. NO BOOT REFUSAL REMAINS. ──
  //
  // THIS IS A DELIBERATE OWNER REVERSAL, recorded so nobody mistakes it for erosion. There is no
  // `if` below because there is no longer anything to refuse: `TEST_IMMEDIATE_DELETE_ENABLED`
  // arms the seam in EVERY environment, production included.
  //
  // The history, because the two removed guards were both argued and both deliberate:
  //
  //   #1187  refused to boot when the flag was true and RAW NODE_ENV was not explicitly
  //          development/test/staging. Rationale: the boolean "left one env var between
  //          production and permanent data loss".
  //   #1264  replaced that with "production permitted, but only alongside a NON-EMPTY
  //          TEST_IMMEDIATE_DELETE_WORKER_IDS", making the blast radius enumerable.
  //   NOW    neither. Owner decision, 2026-08-27: the button deletes the account of WHOEVER
  //          taps it — worker or developer, no distinction — and the gate is which APK carries
  //          the button, not which account is asking.
  //
  // WHAT THAT MEANS IN PLAIN TERMS, so the next reader does not have to reconstruct it: with the
  // flag on in production, ANY authenticated worker can irreversibly destroy their own account
  // and everything attached to it, in one request, with NO step-up OTP and NO ADR-0031 grace.
  // The route is ordinary HTTP, so this is reachable with any valid worker token regardless of
  // which build it came from — a client-side button is not an authorization boundary and the
  // server cannot see which APK sent a request. The practical exposure is therefore account
  // TAKEOVER: a stolen or borrowed token becomes permanent erasure, where the step-up OTP on the
  // real DPDP flow would otherwise have stood in the way.
  //
  // That is understood and accepted. It is not a gap to be quietly re-closed by a later reader:
  // if you believe this should be gated again, RAISE IT WITH THE OWNER rather than adding a
  // refusal here — re-arming a guard the owner removed would silently break the shipped product.
  //
  // TEST_IMMEDIATE_DELETE_WORKER_IDS SURVIVES AS AN OPTIONAL NARROWING, not a requirement.
  // Empty (the default, and the shipped posture) means every authenticated worker. Non-empty
  // restricts the seam to those ids. Nothing forces it to be set, in any environment.
