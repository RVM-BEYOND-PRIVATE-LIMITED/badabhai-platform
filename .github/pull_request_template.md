...
- [ ] **Design-system adherence** (any `apps/payer-web` UI change): no raw hex/px — values resolve from tokens (the `pnpm lint` adherence rule + `pnpm lint:oxlint` enforce this); reviewed against the `frontend-product-engineer` checklist
