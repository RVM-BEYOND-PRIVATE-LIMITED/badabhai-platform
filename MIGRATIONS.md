...
| `0113`        | Prakash   | **APPLIED 2026-09-17** — ADR-0042 D9 / Layer A (e): `worker_portfolio` (kind photo/video/link; storage_key for media, url for links; content CHECK; worker order; RLS ENABLE + hand-appended FORCE + four REVOKEs). New empty table; registered as `0113-worker-portfolio-table` + `0113-worker-portfolio-rls`; listed in the e2e RLS guard. The media BUCKET (`WORKER_PORTFOLIO_BUCKET`) is dormant until infra sets it — the mint route 503s |
